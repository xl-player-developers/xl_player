...
void glMatrixFromSo3(Matrix3x3d *so3, double * matrix)
{
...
void OrientationEKF::getPredictedGLMatrix(double secondsAfterLastGyroEvent, double * matrix)
{
    ...
    glMatrixFromSo3(&so3PredictedState, matrix);
}

void OrientationEKF::processGyro(Vector3d gyro, double sensorTimeStamp)
{
    if (_sensorTimeStampGyro != 0.0) {

        double dT = (sensorTimeStamp - _sensorTimeStampGyro)* 1.0E-09F ;
        if (dT > 0.04f) {
...
void OrientationEKF::processAcceleration(Vector3d acc, __attribute__((unused)) double sensorTimeStamp)
{
...
void OrientationEKF::updateAccelerationCovariance(double currentAccelNorm)
{
    ...
    double accelNoiseSigma = kMinAccelNoiseSigma + normChangeRatio * (kMaxAccelNoiseSigma-kMinAccelNoiseSigma);
    if(kMaxAccelNoiseSigma < kMinAccelNoiseSigma + normChangeRatio * (kMaxAccelNoiseSigma-kMinAccelNoiseSigma)){
