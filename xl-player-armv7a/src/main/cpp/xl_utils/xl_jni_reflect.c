...
#include "xl_jni_reflect.h"

void xl_jni_reflect_java_class(xl_java_class ** p_jc, JNIEnv *jniEnv) {
    xl_java_class * jc = malloc(sizeof(xl_java_class));
...
//    jc->XLPlayer_class = (*jniEnv)->NewGlobalRef(jniEnv, xlPlayerClass);
    ...
    jclass java_HwDecodeBridge = (*jniEnv)->FindClass(jniEnv, "com/xl/media/hwdecode/HwDecodeBridge");
