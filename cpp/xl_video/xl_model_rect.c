...
static inline void resize_video(xl_model *model){
    ...
    if(model->frame_rotation == XL_ROTATION_90 || model->frame_rotation == XL_ROTATION_270){
...
void model_rect_resize(xl_model *model, int w, int h) {
    model->viewport_w = w;
    model->viewport_h = h;
    resize_video(model);
}

static void draw_rect(xl_model *model){
