...
#ifndef XL_XL_MEDIACODEC_H
#define XL_XL_MEDIACODEC_H
...
/*
typedef struct struct_xl_nalu_pos{
    uint32_t nal_len[16];
    uint32_t nal_pos[16];
    uint8_t count;
}xl_nalu_pos;

static void convert_h264_to_annexb2( uint8_t *p_buf, size_t i_len,
                                    size_t i_nal_size,
                                     xl_nalu_pos *state )
{
    ...
    while( i_len > 0 )
    {
        if( state->nal_pos[state->count] < i_nal_size ) {
            unsigned int i;
            for( i = 0; state->nal_pos[state->count] < i_nal_size && i < i_len; i++, state->nal_pos[state->count]++ ) {
                state->nal_len[state->count] = (state->nal_len[state->count] << 8) | p_buf[i];
                p_buf[i] = 0;
            }
            if( state->nal_pos[state->count] < i_nal_size )
                return;
        ...
        if( state->nal_len[state->count] > INT_MAX )
            return;
        if( state->nal_len[state->count] > i_len )
        {
            state->nal_len[state->count] -= i_len;
            return;
        }else
        {
            p_buf += state->nal_len[state->count];
            i_len -= state->nal_len[state->count];
//            state->nal_len = 0;
//            state->nal_pos = 0;
            state->count++;
        }
